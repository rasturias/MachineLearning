...
```
Variables above are highly correlated, therefore, PCA will be used in the pre-processing

## CrossValidation

To avoid overfitting and to reduce out of sample errors, TrainControl is used to perform 7-fold cross validation
...
#Neural Net
NN <- train(classe ~ ., data = TrainSet, method = "nnet", trControl= trainC, verbose=FALSE )
#Bayes Generalized Linear model
...
### Accuracy Results
```{r }
allModels <- c("Random Forest", "SVM","Neural Net","Bayes GLM","LogitBoost")
...
```{r }
#rfPrediction <- predict(rf, testData)
#confMatRandForest <- confusionMatrix(rfPrediction, testData$classe)
#confMatRandForest
